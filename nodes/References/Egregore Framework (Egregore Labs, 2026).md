...
- **Terminal-first via Claude Code as the contribution path.** Every flow in Egregore assumes `claude` is running locally with `gh`, `jq`, and an installed shell alias. There is no GitHub Web UI editing path. This is structurally incompatible with [[Adopt Forkable Publication Model]]'s commitment to the Web UI as a first-class edit path, which exists so non-developers can contribute without local tooling.
- **Two-repo (framework + memory) plus managed-sibling structure.** An Egregore instance is not one git repository; it is a framework repo, a symlinked memory repo, and possibly N other managed repos. DeepContext's commitment is that the unit is one git repo and one live site. There is no "fork the Egregore" path analogous to DeepContext's fork-publishes-itself model.
- **Provision-from-template plus collaborator-invite, instead of fork.** A new contributor typically joins an existing instance via `/invite` (which calls the GitHub collaborator API to add them to every managed repo) rather than forking to produce their own. Standing up a new instance is positioned as an org-founding act, not an individual-contributor act. DeepContext's model is the inverse: forking is the ongoing practice, not initial setup.
...
- **High-ceremony branching and integration-branch discipline.** `/save` opens PRs against a `develop` integration branch; the branch-guard hook mandates worktrees derived from `dev/{author}/{slug}`. DeepContext's commitment is simpler: main is the live site. Layering in topic-branching-via-worktree would contradict the friction floor [[Adopt Forkable Publication Model]] commits to.
...
- informs_downstream::[[Adopt Forkable Publication Model]]
  - DeepContext's unit-of-participation (a git repository that, when forked, produces its own live site with first-class Web UI editing) differs structurally from Egregore's unit (a provisioned-from-template instance with collaborator-invite, terminal-first Claude Code requirement, and framework-sync-via-checkout). The Decision's alternatives section names the Egregore-shape as the specific model the forkability commitment diverges from.
