...
- **High-ceremony branching and integration-branch discipline.** `/save` opens PRs against a `develop` integration branch; the branch-guard hook mandates worktrees derived from `dev/{author}/{slug}`. DeepContext's commitment is simpler: main is the live site. Layering in topic-branching-via-worktree would contradict the friction floor [[Adopt Self-Sovereign Graph Publication]] commits to.
