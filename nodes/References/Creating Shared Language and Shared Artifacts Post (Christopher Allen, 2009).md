---
created: 2026-04-20
tagline: Christopher Allen's 2009 post on groups intentionally developing shared language through collaborative artifacts — the earlier foundational framing underneath this project's convention layer
brief_summary: Life with Alacrity blog post from 2009-09-17 arguing that groups must intentionally develop shared language for effective collaboration, and that creating "shared artifacts" (collaboratively-developed objects — models, documents, diagrams) is among the most effective mechanisms for facilitating that development. The post's stance that meaning is provided by people who use words (not inherent in the words themselves) informs this project's author-declared-edges discipline. Specific facilitation techniques (napkin-drawing, workshop practices) are not adopted; the architectural principle (intentional shared-language development through collaborative artifact creation) carries forward as substrate for the project's convention layer and for the shared-artifact role the graph itself plays.
---
