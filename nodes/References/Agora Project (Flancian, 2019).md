...
- **Per-contributor does not publish.** A contributor registering a garden in someone else's Agora does not get their own rendered surface from that registration. This differs from DeepContext's commitment that a fork's first push builds the fork's own site.
...
- Agora's unit of participation is a garden registered in an aggregator's manifest; the garden does not produce a live site on its own. This is the structural contrast with DeepContext's fork-publishes-itself commitment that [[Adopt Forkable Publication Model]] makes.
- The aggregator-plus-many-gardens model and the forkable-single-repo model are not competitors — they address different scales. Agora's patterns are worth naming for the later phase where cross-graph aggregation becomes load-bearing; the MVA Stance defers building until that need surfaces.
...
- informs_downstream::[[Adopt Forkable Publication Model]]
  - The Decision commits to a fork-publishes-itself unit of participation that differs structurally from Agora's aggregator-plus-gardens model. This Reference is one of the precedents the Decision contrasts with — Agora's model shows what aggregation-first federation looks like, and the Decision names why DeepContext's current scope chose self-contained publication instead.
