...
- **Shell-only tooling for the inception ceremony.** OI ships `create_inception_commit.sh`, `audit_inception_commit-POC.sh`, and `get_repo_did.sh` as zsh scripts. DeepContext reimplements the ceremony in Python to preserve the standard-library-plus-one-dependency commitment. The reimplementation tracks the specification's cryptographic requirements (empty tree, SSH signature, canonical commit message, SHA-256 committer name) without introducing a non-Python dependency.
