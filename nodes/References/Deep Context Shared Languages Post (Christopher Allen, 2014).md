---
created: 2026-04-20
tagline: Christopher Allen's 2014 post naming "deep context" as the layered meaning that community-specific vocabularies carry — the origin of this project's architectural name
brief_summary: Life with Alacrity blog post from 2014-06-18 that introduces "deep context" as the layered understanding beneath a community's specialized vocabulary — the shared language that carries tribal bonding power and conceptual compression for practitioners. This project takes its architectural name from this post. The post's framing of shared languages as structural "machinery" rather than surface-level signifiers supports the project's Vocabulary Diversity Is a Feature Conviction. The post's specific examples (Glass Bead Game synthesis, Solution Box quadrant model, hashtag-as-shortcut) are not adopted directly — the project's convention layer uses wikilinks and named-edge predicates for the compression role that hashtags play in the post.
---
...
- **The Solution Box quadrant model.** The four-quadrant positioning framework (INSIGHT, PHILOSOPHY, SCHEMATIC DESIGN, and so on) is a specific maturity-level device. The project uses `has_lifecycle::` and `has_curation::` predicates for its maturity axis rather than a fixed quadrant model.
- **Hashtag-as-shortcut as the primary compression mechanism.** The post frames #Cynefin, #Intrapreneuring, and similar hashtags as the compression layer for deep context. This project uses wikilinks and named-edge predicates (`[[Target]]`, `predicate::[[Target]]`) for the compression role — structural, typed, navigable, and preserving direction. The distinction matters: hashtags flatten into co-occurrence bags; wikilinks build a navigable graph.
