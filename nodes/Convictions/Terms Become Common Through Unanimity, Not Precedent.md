...
The risk is most concrete at the beginning of a project, when the graph's founding vocabulary gets chosen. [[Founding Vocabularies Constrain Later Participants]] records the dynamic: architects who pick terms early end up constraining users later, and the constraint is often invisible until a newcomer encounters the vocabulary and finds no way to reshape it. One known mechanism that operationalizes the unanimity stance is a three-state process: a term moves from one participant's vocabulary into the shared Common vocabulary only through an explicit step where each participant chooses Affirm, No objection, or Object. A single Object blocks the move; Affirm and No objection both count toward adoption. The mechanism treats silence-as-consent as insufficient -- a term becoming common requires participants to at least not object, which is a weaker bar than affirmation but stronger than precedent.

Deep Context adopts the stance at the abstraction level rather than the specific mechanism. Unanimity-over-precedent is the commitment; the three-state process is one way to implement it. A scion operating at a different scale, or in a different cultural context, could implement the commitment through a different mechanism -- periodic Common-vocabulary reviews, explicit proposals in a dedicated location, or a convention that new common terms must be proposed by someone other than the term's originator. The scion pattern itself depends on this -- a scion inherits the vocabulary the template already converged on, but the scion's own new common terms are the scion's participants' to establish, not the template's to project downstream.
