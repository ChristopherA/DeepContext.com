---
created: 2026-04-20
tagline: When vocabularies meet, both edges land as distinct claims and an agent translates between them — convergence flattens what translation preserves
brief_summary: A held stance specializing Vocabulary Diversity Is a Feature with a concrete operational rule for the contact surface between vocabularies. When two vocabularies meet across contributors or systems, both `critiques::[[X]]` and `challenges::[[X]]` land in the graph as distinct claims; a facilitating agent translates between them rather than collapsing them into a normalized `disagrees_with::`. Mutual intelligibility through translation is the cross-vocabulary target rather than convergence on a shared ontology — because convergence has three structural costs (consensus creates priesthoods, standardized language becomes dogma, flattening destroys encoded meaning) that the translation-preserving alternative avoids.
---
...
- informs_downstream::[[source_voice -- whose thinking is captured when different from the author]]
...
- informs_downstream::[[responds_to -- direct reply creating a conversation thread]]
  - The conversation-threading predicate adopted from the same adjacent corpus. Preserves the distinction between a node that grounds in another and a node that replies to another as part of a dialogic thread — again, translation-not-convergence rather than merging into `informs_downstream::` or `grounded_in::`.
