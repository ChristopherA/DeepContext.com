...
The commitment is scoped to prose flow. Structural contexts — `## Relations` edges, `### Against [[adjacent predicate -- sense]]` H3 subsection headings in Predicate nodes, and other index-shaped locations where a reader scans rather than reads running prose — permit the bare full filename `[[full filename -- sense]]` as an alternative. In those contexts the ` -- sense` suffix is carrying the target's working definition to the reader at the edge site, and the bare full form is informative rather than distracting. The forcing case this Decision cared about — prose that strings full gloss filenames into running text and becomes unscannable — does not apply to structured indices. The prohibition on the bare concept form `[[concept]]` still applies everywhere, because it does not resolve in standard wiki tooling.
