...
The commitment also follows from [[Adopt Forkable Publication Model]]. A fork inherits the graph; if skills live outside the graph, the fork inherits a graph whose maintenance workflows live somewhere the fork owner has to assemble separately. Putting skills inside the graph makes the fork self-contained at the capability layer the same way the forkability commitment makes it self-contained at the publication layer. A fork inheriting the repository inherits the skills that maintain it, without external dependencies.
...
**A fork's skills diverge from the parent's so substantially that co-location ceases to matter.** If forks consistently replace most of the parent's skill set rather than extending it, the benefit of skills traveling with the repository is weaker -- what the fork inherits is not being used. The revisit would ask whether skills should be fork-specific artifacts declared separately rather than graph-resident nodes shared across forks by default.
