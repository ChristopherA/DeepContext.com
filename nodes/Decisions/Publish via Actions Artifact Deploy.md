...
brief_summary: DeepContext.com publishes via GitHub Pages' Actions artifact deployment, not via branch deployment with committed build output. The pipeline runs on every push to main, produces .build/ in the runner, uploads it as a Pages artifact, and deploys it through actions/deploy-pages. The .build/ directory is gitignored; no generated HTML lives in git. The Decision records a mid-execution pivot -- the initial Product Requirements Document (PRD) committed to .build/ being committed to main and served via branch deployment, but GitHub Pages' branch-deploy source only accepts '/' or '/docs' as folder options. Renaming to /docs was rejected as semantically overloaded; switching to Actions artifact deploy keeps the .build/ name, cleans git history of generated output, and drops the loop-prevention mechanics (paths-ignore, bot-author guard) that committed-build required.
---
...
The cost of switching is that first-time scion owners have to do one more configuration step. Branch-deploy scions need "Source = Deploy from a branch; Folder = /docs or /." Actions-deploy scions need "Source = GitHub Actions" plus a one-time Actions enablement (Actions are disabled by default for new repositories created from templates, for security). Both are one-time clicks in Settings. The Actions-enable requirement is the meaningful friction increment; the Pages-source change is comparable to branch-deploy.
...
**Actions requirement becomes a meaningful friction floor for scions.** If the data shows scions commonly fail to enable Actions (leaving their Pages site as a 404) the friction is blocking rather than a one-time setup cost. A revisit would reconsider committed-build with `/docs` renaming or a branch-deploy flow that scion owners can enable with fewer clicks.
...
**A scion's independent pipeline diverges from the template's.** If a scion modifies the pipeline in ways that break the Actions-deploy contract, and the template wants to support scion-with-custom-pipeline without imposing conventions on the scion, a different architecture (e.g., scion brings its own workflow) might be preferable. This is a scion edge case the current Decision doesn't resolve.
