...
If the output is empty, no signing key is configured. Check whether the user already has an SSH key on disk:

```sh
ls ~/.ssh/id_ed25519.pub ~/.ssh/id_rsa.pub 2>/dev/null
```

If a key exists and the user wants to use it for signing, configure git:
...
```sh
ssh-keygen -t ed25519 -f ~/.ssh/id_ed25519 -C "$(git config user.email)"
git config --global gpg.format ssh
git config --global user.signingkey "$HOME/.ssh/id_ed25519.pub"
```

The public key must be registered with GitHub as a **signing key** (not just an auth key). Add it via `gh`:
