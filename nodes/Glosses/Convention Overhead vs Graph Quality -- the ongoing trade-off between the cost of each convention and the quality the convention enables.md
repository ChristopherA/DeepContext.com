...
The project does not treat one side of the trade-off as paramount. Self-sovereign graph publication depends on the repository's surface staying small enough that any first steward can understand and stand up their own graph; the practice's durability depends on the conventions being strong enough to survive drift. Both pressures are real. The trade-off is where the project does its architectural thinking -- every Contract with a `contends_with::` edge to this concept is a node whose author considered the cost alongside the benefit and decided the benefit carried the day, with the `contends_with::` edge acknowledging that a future reader may weigh the same trade-off differently.
