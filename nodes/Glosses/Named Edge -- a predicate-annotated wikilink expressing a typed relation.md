...
Named Edges appear in two places. The **identity predicate block** above the H1 carries classification edges — `conforms_to::[[X Form Contract]]`, `in_practice_domain::[[Y]]`, `has_lifecycle::[[Z Stage]]` — that declare what the node is. The **Relations section** after the body carries semantic edges — `composed_of::`, `contends_with::`, `informs_downstream::` — that declare how the node connects to others. Both use the same syntax; their roles differ by position in the file, not by form.
