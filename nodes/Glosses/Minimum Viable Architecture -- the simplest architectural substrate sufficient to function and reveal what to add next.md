...
- composes_with::[[Convention Overhead vs Graph Quality -- the ongoing trade-off between the cost of each convention and the quality the convention enables]]
  - Both Glosses name disciplines for evaluating proposed structural additions. Convention Overhead vs Graph Quality is the trade-off; Minimum Viable Architecture is the discipline applied to that trade-off in first-cycle work -- when the trade-off is unclear, defer.
