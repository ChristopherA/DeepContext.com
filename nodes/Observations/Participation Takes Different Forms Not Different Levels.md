---
created: 2026-04-20
tagline: Volume alone mismeasures participation — contributors whose work takes the form of responding or curating rather than authoring register as light participants under volume-only measures while carrying first-class participatory loads under form-sensitive measures
brief_summary: A Retrospective Observation grounded in a four-lens graph analysis of a plural-contributor knowledge corpus. The measurement uses four axes — authoring volume, response density, topic diversity, and temporal cadence — and surfaces a finding volume-only measurement would miss: in a measured case, one contributor carried high authoring volume with low response density while another carried high response density with moderate authoring volume. Volume-only measurement collapses these orthogonal modalities into a single scale; form-sensitive measurement surfaces them as distinct participatory loads of comparable weight. The observation generalizes beyond any one corpus to collaborative communities where forms other than authoring carry load, and names volume-only measurement as a measurement artifact rather than the underlying structure of collaboration.
---
...
- informed_by::[[Second-Cycle Contributors Are the Scarce Resource]]
  - The scarce-resource Observation carries the power-law distributional finding inside its Grounds — shape durable across platforms, ratios community-specific. This Observation names one of the reasons volume-only measurement of that distribution is load-bearing-but-incomplete: the shape is durable under volume-only measurement but would look different under form-sensitive measurement. The two are compatible — power-law distributions can be real at the authoring-volume axis and coexist with form-plurality on other axes.
