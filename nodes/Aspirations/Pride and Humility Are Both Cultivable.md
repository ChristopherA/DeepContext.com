---
created: 2026-04-20
tagline: The practice develops the specific conditions that help contributors simultaneously feel their contribution is worthwhile and accept that others can improve it — both traits are cultivable through practice design, not given as preconditions
brief_summary: An Aspiration targeting the design surface where the trait-pair named in Meaningful Wiki Contribution Requires Both Pride and Humility becomes cultivable rather than preconditional. Current conventions specify what to author (forms, predicates, annotations) but not how to develop the psychological and social conditions for authoring to feel worthwhile and revisable. The work is to design affirmation practices that validate contribution before critique, revision rituals that normalize "others improve mine" as expected participation, and curation practices visible to contributors so revision is felt as participation in a shared practice rather than as rejection. Grounded in the pride-humility Observation and in Vocabulary Diversity Is a Feature — the sovereignty stance that contributor vocabularies are preserved extends to the stance that contributions are the contributor's and revision is collaboration, not correction.
---
...
The cultivation frame is distinct from selection. Selection rewards contributors who already hold the traits; cultivation shapes the practice so that contributors who hold one or neither can develop both. Selection narrows the contributor population; cultivation widens it. A community that targets the second cycle (`[[The Second Cycle of Contribution Happens]]`) and the first-to-second-cycle contributor transition the Aspiration's contributor-level work addresses needs cultivation rather than selection — selection filters at entry, and the losses compound across cycles.
...
Curation output is not visible to contributors between cycles. `[[The Second Cycle of Contribution Happens]]` names this gap in its contributor-level transition work from the first-to-second-cycle angle; the trait-cultivation reading of the same gap is that contributors cannot develop humility toward revision if they do not see revision as an ongoing practice they participate in. Invisibility turns revision into an external operation rather than a shared one.
...
Establish curation practices visible to contributors so being revised is felt as participation in a practice rather than as rejection. The curation-output-as-contributor-surface work described in `[[The Second Cycle of Contribution Happens]]`'s contributor-level transition is the mechanism; the trait-cultivation Aspiration names the purpose — visibility is what lets contributors develop humility toward revision as an ordinary practice.

Consider whether contribution recognition can be built into the graph's own structure. `first_authored_by::` predicates preserved through revision, so the original author is still visible even when the node has evolved; `cited_by::` aggregation so contributors see what their contributions have come to ground; per-contributor summary surfaces that aggregate a contributor's graph footprint. The specifics are open and belong as candidate Predicate nodes in `nodes/Predicates/`; the commitment is that recognition becomes part of the graph rather than a separate acknowledgment system.
...
- `[[Meaningful Wiki Contribution Requires Both Pride and Humility]]` — the grounding Observation that identifies the trait-pair as a partial account of the newcomer-contribution threshold.
- Caulfield, Mike. *Choral Explanations*, 2016 — <https://hapgood.us/2016/05/13/choral-explanations/>↗. An alternate model where parallel explanations reduce the humility burden by not requiring overwrite; relevant to the design of revision rituals that would cultivate humility without demanding consensus.

## Relations

- informed_by::[[Meaningful Wiki Contribution Requires Both Pride and Humility]]
  - The Observation names the trait-pair and the threshold; this Aspiration targets the practice's design response. The Observation is retrospective — describing what is — and the Aspiration is directional — targeting what the practice works toward.
...
- composes_with::[[LLM Assistance Widens the Participation Gap]]
  - The two-gate reading in `[[Meaningful Wiki Contribution Requires Both Pride and Humility]]` says membership fluency (the gate in the LLM Observation) and the trait-pair are orthogonal gates. This Aspiration addresses the trait-layer gate; a fuller practice-level response would also address the register-and-culture-layer gate. The two compose; neither fully captures what the other targets.
