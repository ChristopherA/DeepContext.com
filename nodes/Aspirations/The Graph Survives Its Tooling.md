---
created: 2026-04-20
tagline: The graph remains readable, traversable, and authorable across editor, wiki-layer, agent, and build-system changes — no tool becomes load-bearing for the graph's meaning
brief_summary: An Aspiration that the graph's meaning survives the tools that helped produce it. The plain-markdown floor holds across tool changes — editor changes, wiki-layer changes, agent changes, build-system changes — and no tool becomes load-bearing for the graph's interpretation. The Aspiration is the specific test case for whether the zero-tooling-floor commitments in Adopt Wikilinks and Named Edges, Restrict YAML to Scalar Metadata, and Keep URLs as Scalar Metadata Not Predicates sustain across the generational timeline a shared knowledge graph implies. The gap is concrete: the graph has been authored under one editor configuration and one agent harness, and tool-independence has not been tested.
---
